...
Citizen.CreateThread(function()
	...
	while NVCore.Functions.GetPlayerData().job == nil do
		Citizen.Wait(10)
	end
	PlayerData = NVCore.Functions.GetPlayerData()
end)

RegisterNetEvent('NVCore:Client:OnPlayerLoaded')
AddEventHandler('NVCore:Client:OnPlayerLoaded', function()
    NVCore.Functions.GetPlayerData(function(PlayerData)
...
RegisterNetEvent('NVCore:Client:OnJobUpdate')
AddEventHandler('NVCore:Client:OnJobUpdate', function(JobInfo)
							...
							if hasBeenCaught == false then
								...
								-- VEHICLES ABOVE ARE BLACKLISTED
									...
									if useFlashingScreen == true then
										TriggerServerEvent('nv-speedcamera:openGUI')
									end
									...
									if useFlashingScreen == true then
										Citizen.Wait(200)
										TriggerServerEvent('nv-speedcamera:closeGUI')
									end
								
									NVCore.Functions.Notify("you have been fined for speed exceeding maximum limit exceeded: 60KM", "error")
									
									if useBilling == true then
                                        -- OLD esx
									else
										TriggerServerEvent('nv-speedcamera:PayBill60Zone')
						...
						if (GetPedInVehicleSeat(playerCar, -1) == playerPed) then					
							if hasBeenCaught == false then
								...
								-- VEHICLES ABOVE ARE BLACKLISTED
									...
									NVCore.Functions.Notify("you have been fined for speed exceeding maximum limit exceeded: 80KM.", "error")
									
									if useBilling == true then
                                        -- old ESX function
									else
										TriggerServerEvent('nv-speedcamera:PayBill80Zone')
						...
						if (GetPedInVehicleSeat(playerCar, -1) == playerPed) then 
							if hasBeenCaught == false then
								...
								-- VEHICLES ABOVE ARE BLACKLISTED
									...
									NVCore.Functions.Notify("you have been fined for speed exceeding maximum limit exceeded: 120KM.", "error")
									
									if useBilling == true then
                                        -- OLD ESX
									else
										TriggerServerEvent('nv-speedcamera:PayBill120Zone')
